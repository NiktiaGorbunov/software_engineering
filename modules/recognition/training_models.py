...
def train_model_py_img(name_user: str):
    ...
    for i, image in enumerate(images):
        ...
        face_img = face_recognition.load_image_file(
            f"dataset/users_img/{name_user}/{image}"
        )
...
def take_screenshot(name_user: str):
    ...
    while True:
        ...
        if ret:
            ...
            k = cv2.waitKey(
                20
            )  # регулирем скорость воспроизведения видео и что-то про клавиши

            if k == ord("s"):
                cv2.imwrite(
                    f"dataset/users_img/{name_user}/{count}_screen.jpg", frame
                )  # сохраняем скриншоты
